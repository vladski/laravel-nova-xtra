export default class NovaXtraClass {
    ...
    constructor() {
        this.version = 2
        this.apiBasePath = '/nova-xtra-api/';
        ...
        this._modalTpl =
            ...
            '<div class="relative mx-auto max-w-lg flex justify-center z-20 py-view">' +
                '<div class="relative w-full mx-8 bg-white rounded-lg shadow-lg overflow-hidden">' +
                    '<div class="nxtra_modal_content"></div>' +
                    '<div class="nxtra_modal_loader hidden absolute pin bg-white z-20 opacity-75 text-center">' +
                        '<svg width="69" height="72" viewBox="0 0 23 24" xmlns="http://www.w3.org/2000/svg" class="spin fill-80 m-8"><path d="M20.12 20.455A12.184 12.184 0 0 1 11.5 24a12.18 12.18 0 0 1-9.333-4.319c4.772 3.933 11.88 3.687 16.36-.738a7.571 7.571 0 0 0 0-10.8c-3.018-2.982-7.912-2.982-10.931 0a3.245 3.245 0 0 0 0 4.628 3.342 3.342 0 0 0 4.685 0 1.114 1.114 0 0 1 1.561 0 1.082 1.082 0 0 1 0 1.543 5.57 5.57 0 0 1-7.808 0 5.408 5.408 0 0 1 0-7.714c3.881-3.834 10.174-3.834 14.055 0a9.734 9.734 0 0 1 .03 13.855zM4.472 5.057a7.571 7.571 0 0 0 0 10.8c3.018 2.982 7.912 2.982 10.931 0a3.245 3.245 0 0 0 0-4.628 3.342 3.342 0 0 0-4.685 0 1.114 1.114 0 0 1-1.561 0 1.082 1.082 0 0 1 0-1.543 5.57 5.57 0 0 1 7.808 0 5.408 5.408 0 0 1 0 7.714c-3.881 3.834-10.174 3.834-14.055 0a9.734 9.734 0 0 1-.015-13.87C5.096 1.35 8.138 0 11.5 0c3.75 0 7.105 1.68 9.333 4.319C16.06.386 8.953.632 4.473 5.057z" fill-rule="evenodd"></path></svg>' +
                    '</div>' +
                    '<div class="nxtra_modal_error hidden text-center text-white px-3 py-3 bg-danger-dark"></div>' +
                    '<div class="nxtra_modal_close w-8 h-8 m-2 absolute pin-r pin-t text-50 hover:text-80"><svg fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12"></path></svg></div>' +
                '</div>' +
            '</div>';
        this.init();
    }
    ...
    init() {
        ...
        this.modal.classList.add('nxtra-modal','hidden','modal','select-none','fixed','pin','z-50','overflow-x-hidden','overflow-y-auto','text-left');
        this.modal.innerHTML = this._modalTpl;
        document.body.appendChild(this.modal);

        this._modalContent = this.modal.querySelector('.nxtra_modal_content');
        this._modalLoader = this.modal.querySelector('.nxtra_modal_loader');
        this._modalOverlay = this.modal.querySelector('.nxtra_modal_overlay');
        this._modalClose = this.modal.querySelector('.nxtra_modal_close');
        this._modalError = this.modal.querySelector('.nxtra_modal_error');
    ...
    openModal(routePath) {
        ...
        if (routePath) {
            ...
            Nova.request()
                .get(`${this.apiBasePath}${routePath}`)
    ...
    submitModal (params) {
        ...
        this.errorModal()
        Nova.request()
            .post(`${this.apiBasePath}${params.routePath}`)
            .then(response => { return response.headers['content-type'] === 'application/json' ? response : Promise.reject(response); })
            .then(response => {
    ...
    fetchContent (params) {
        ...
        Nova.request()
            .get(`${this.apiBasePath}${params.routePath}`)
            .then(response => { return response.headers['content-type'] === 'application/json' ? response : Promise.reject(response); })
            .then(response => {
            ...
            case 'detail':
                ...
                if(resourceName && resourceId){
                    ...
                    if (component) {
                        ...
                        Nova.success(`Reloading details`)
                    }
                }
                break
            case 'nova-xtra-page':
                ...
                if(slug){
                    ...
                    if (component) {
                        ...
                        Nova.success(`Reloading page`)
                    }
                }
                break
            default:
                Nova.error(`Reloading not supported for route ${route}`)
